...
class Cov3dLoss(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.neighbour_smoothing:
            self.neighbour_smoothing_weights = (
                torch.as_tensor(
                    [
                        [
                            [
        ...
        if self.severity_everything:
            ...
            n_negative = 1110
            n_positive = (
                total - n_negative - n_critical - n_severe - n_moderate - n_mild
            )
            ...
            self.target_mild = (
                torch.as_tensor([big, little, 0.0, 0.0, little]).to(self.device)
                * self.mild_weight
            )
            self.target_moderate = (
                torch.as_tensor([little, big, little, 0.0, 0.0]).to(self.device)
                * self.moderate_weight
            )
            self.target_severe = (
                torch.as_tensor([0.0, little, big, little, 0.0]).to(self.device)
                * self.severe_weight
            )
            self.target_critical = (
                torch.as_tensor([0.0, 0.0, self.severity_smoothing, big, 0.0]).to(
                    self.device
                )
            ...
            self.target_negative = (
                torch.as_tensor([self.severity_smoothing, 0.0, 0.0, 0.0, big]).to(
                    self.device
                )
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        smoothed_labels = (
            presence_labels * (1.0 - self.presence_smoothing * 2.0)
            + self.presence_smoothing
        )
        presence_loss = F.binary_cross_entropy_with_logits(
            input[:, :1], smoothed_labels, pos_weight=self.pos_weight
        )
        ...
        if self.severity_everything:
            ...
            if torch.all(positive_cases):
                ...
                severity_probabilities = torch.softmax(
                    input[positive_cases, 1:], dim=-1
                )
                severity_loss += -torch.log(
                    severity_probabilities[:, :-1].sum(dim=-1)
                ).sum()
                ...
                negative_cases = (non_positive_targets[:, 1] == 0) & (
                    non_positive_targets[:, 0] == 0
                )

                if torch.any(mild_cases):
                    severity_target[mild_cases, :] = self.target_mild.repeat(
                        mild_cases.sum(), 1
                    )
                    weights += mild_cases.sum() * self.mild_weight
                if torch.any(moderate_cases):
                    severity_target[moderate_cases, :] = self.target_moderate.repeat(
                        moderate_cases.sum(), 1
                    )
                    weights += moderate_cases.sum() * self.moderate_weight
                if torch.any(severe_cases):
                    severity_target[severe_cases, :] = self.target_severe.repeat(
                        severe_cases.sum(), 1
                    )
                    weights += severe_cases.sum() * self.severe_weight
                if torch.any(critical_cases):
                    severity_target[critical_cases, :] = self.target_critical.repeat(
                        critical_cases.sum(), 1
                    )
                    weights += critical_cases.sum() * self.critical_weight
                if torch.any(negative_cases):
                    severity_target[negative_cases, :] = self.target_negative.repeat(
                        negative_cases.sum(), 1
                    )
                    weights += negative_cases.sum() * self.negative_weight

                severity_loss += (
                    (-severity_target * F.log_softmax(severity_predictions, dim=-1))
                    .sum(dim=-1)
                    .sum()
                )
            ...
            if severity_present.sum() > 0:
                if self.severity_regression:
                    ...
                    if self.mse:
                        severity_target_logits = torch.logit(
                            severity_probability_labels
                        )
                    ...
                    severity_target = (
                        target[severity_present, 1] - 1
                    )  # The minus one is because the labels are 0–4 and we want to ignore the zero class
                    if self.neighbour_smoothing:
                        severity_target = F.one_hot(
                            severity_target, num_classes=4
                        ).half()  # hack
                        severity_target = F.pad(
                            severity_target, (1, 1), mode="replicate"
                        )
                        severity_target = torch.unsqueeze(severity_target, dim=1)
                        severity_target = F.conv1d(
                            severity_target, self.neighbour_smoothing_weights
                        )
                        ...
                        severity_loss = (
                            (
                                -severity_target
                                * F.log_softmax(input[severity_present, 1:], dim=-1)
                            )
                            .sum(dim=-1)
                            .mean()
                        )
        ...
        return (
            presence_loss * (1.0 - self.severity_factor)
            + self.severity_factor * severity_loss
        )
